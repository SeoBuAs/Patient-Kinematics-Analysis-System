...
class CompleteExperiment:
    ...
    def run_nested_cv_for_model(self, model_name, model_info, X_train, y_train, X_val, y_val, X_test, y_test):
        logger.info(f"\n=== {model_name} model with fixed validation set ===")
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv_outer.split(X_train, y_train), 1):
            ...
            grid_search.cv = PatientBasedCV(random_state=42, n_splits=5)
        ...
        final_grid_search.cv = PatientBasedCV(random_state=42, n_splits=5)
        final_grid_search.fit(X_train, y_train)
        y_test_pred = final_grid_search.predict(X_test)
        y_test_pred_proba = final_grid_search.predict_proba(X_test)[:, 1]
    ...
    def run_complete_experiment(self):
        logger.info("=== Starting complete patient-based experiment with fixed validation set ===")
    ...
    def save_results(self):
        ...
        self.experiment_dir = f"results/experiment_{timestamp}"
    ...
    def save_csv_results(self):
        ...
        summary_path = os.path.join(self.experiment_dir, "model_summary.csv")
...
if __name__ == "__main__":
    ...
    logger.info("Complete experiment finished!") 